...
const fromDb = row => ({ id: row.id, category: row.category, name: row.name, description: row.description || '', price: Number(row.price) || 0, unit: row.unit || 'Unidade', minQuantity: Math.max(1, Number(row.min_quantity) || 1), image: row.image_url || '', available: Boolean(row.available), featured: Boolean(row.featured) });
const toDb = p => ({ ...(typeof p.id === 'string' && p.id.includes('-') ? { id: p.id } : {}), category: p.category, name: p.name, description: p.description || '', price: Number(p.price) || 0, unit: p.unit || 'Unidade', min_quantity: Math.max(1, Number(p.minQuantity) || 1), image_url: p.image || null, available: Boolean(p.available), featured: Boolean(p.featured) });
...
function App() {
  ...
  const loadProducts = async () => {
    ...
    if (!error && data) setProducts(data.map(fromDb));
  ...
  useEffect(() => {
    let mounted = true;
    supabase.auth.getSession().then(({ data: { session: currentSession } }) => { if (mounted) { setSession(currentSession); setAuthLoading(false); } });
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, currentSession) => { setSession(currentSession); setAuthLoading(false); if (!currentSession) setAdmin(false); });
  ...
  const save = async next => {
    const normalized = next.map(p => ({ ...p, price: Number(p.price) || 0, minQuantity: Math.max(1, Number(p.minQuantity) || 1) }));
    const dbRows = normalized.map(toDb);
    const existingIds = products.map(p => p.id).filter(id => typeof id === 'string' && id.includes('-'));
    const nextIds = normalized.map(p => p.id).filter(id => typeof id === 'string' && id.includes('-'));
    const removedIds = existingIds.filter(id => !nextIds.includes(id));
    if (removedIds.length) { const { error } = await supabase.from('products').delete().in('id', removedIds); if (error) { alert(`Não foi possível excluir o produto: ${error.message}`); return false; } }
    if (dbRows.length) { const { error } = await supabase.from('products').upsert(dbRows, { onConflict: 'id' }); if (error) { alert(`Não foi possível salvar o produto: ${error.message}`); return false; } }
    await loadProducts();
    return true;
  };
  ...
  return <>
    ...
    <main id="inicio">
      <section className="hero"><div className="hero-copy"><p className="eyebrow">FEITO COM CARINHO • VITORINO - PR</p><h1>Mais do que doces, entregamos <i>felicidade.</i></h1><p className="hero-text">Bolos, doces e sobremesas feitos com muito carinho para tornar o seu dia mais doce.</p><div className="hero-buttons"><a href="#cardapio" className="primary">Ver cardápio <ChevronRight size={18}/></a><a href={`https://wa.me/${WA}`} target="_blank" rel="noreferrer" className="secondary"><MessageCircle size={18}/> Falar pelo WhatsApp</a></div></div><div className="hero-photo"><img src="https://images.unsplash.com/photo-1578985545062-69928b1d9587?auto=format&fit=crop&w=1200&q=90"/><div className="hero-badge"><Heart size={15} fill="currentColor"/><span>Feito com amor</span></div></div></section>
