(() => {
  ...
  const generatePremiumPdf = () => {
        ...
        <div class="products-grid">
          ${items.map(p => `
            <article class="product">
              <div class="photo-wrap"><img src="${esc(p.image)}" alt="${esc(p.name)}" onerror="this.style.display='none'"></div>
              <div class="product-body">
                <div class="product-top"><h3>${esc(p.name)}</h3><span class="price">R$ ${Number(p.price || 0).toFixed(2).replace('.', ',')}</span></div>
                <p>${esc(p.description)}</p>
                <div class="product-meta"><span>${esc(p.unit || '')}</span>${Number(p.minQuantity) > 1 ? `<span>Pedido mínimo: ${Number(p.minQuantity)}</span>` : ''}</div>
              </div>
            </article>`).join('')}
        </div>
      </section>`).join('');
    ...
    w.document.write(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><title>Cardápio | Doces e Delícias da Maristela</title>
      <style>
  ...
  const bind = () => {
    const button = document.querySelector('.admin-modal .pdf-menu-button');
    if (!button || button.dataset.premiumBound) return;
